...
/// програма, який це платіж із дев'яти) і канал зв'язку — edu@uimp.com.ua.
///
...
export async function sendYearlyProgramUpcomingChargeEmail(args: {
  ...
  chargeAt: Date;
  /// Порядковий номер майбутнього списання і скільки всього — null, якщо порахувати
  /// не вдалось (тоді рядок прогресу в лист просто не потрапляє).
  ...
  const progressLine = chargeProgress
    ? `<p style="margin: 0 0 16px; color: #555;">Це буде списання ${chargeProgress.current} з ${chargeProgress.total}.</p>`
    : '';
