...
export default function ArticleTemplate({ data, showHero = true }: Props) {
  return (
    <article className="article-template" style={{ width: "100%", maxWidth: 920, margin: "0 auto", fontFamily: "Inter, system-ui, -apple-system, sans-serif", color: "#1C1917" }}>
      ...
      <header style={{ marginTop: showHero && data.cover.url ? 40 : 0, marginBottom: 32, textAlign: "center", maxWidth: 720, marginLeft: "auto", marginRight: "auto" }}>
        {data.category && (
      ...
      {/* Conclusion */}
      {data.conclusion && (
        <section style={{ maxWidth: 720, margin: "0 auto", marginBottom: 40 }}>
          <h2 style={{ fontSize: 30, fontWeight: 700, lineHeight: 1.25, margin: "0 0 20px", color: "#1C1917" }}>
            Висновки
          </h2>
      ...
      {/* Author / footer line */}
      {data.authorLine && (
        <p style={{ textAlign: "center", fontSize: 13, color: "#78716C", margin: 0, paddingBottom: 8 }}>
          {data.authorLine}
        </p>
      )}
    </article>
