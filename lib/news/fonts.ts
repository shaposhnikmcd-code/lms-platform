// Каталог шрифтів для новин — ЄДИНЕ джерело правди (admin-білдер + public /news).
// Дзеркало UX MS Word, але з вебівських шрифтів, відібраних за критеріями:
...
//
// Шрифти вантажаться через Google Fonts CDN (`<link rel="stylesheet">`). Раніше
// цей лінк інжектився ЛИШЕ в admin-білдері (NewsEditor/TemplateEditor), тож на
// public /news вибраний шрифт не завантажувався — текст падав на fallback.
// Тепер той самий лінк рендериться і на публіці (app/[locale]/news/layout.tsx),
// а домени googleapis/gstatic додані у CSP (next.config.mjs). Family-string у
// `value` — це готова CSS font-family із fallback-ланцюгом.
 ...
 * `display=swap` — текст видно одразу, без FOIT. Один request на всі шрифти —
 * Google API оптимізує payload; браузер лениво тягне woff2 лише для family,
 * реально застосованих у DOM, тож повний bundle на публіці недорогий.
 */
