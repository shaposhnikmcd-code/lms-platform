    ...
    return (
      <footer className="bg-[#1C3A2E] text-white">
        <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-white/60">
          <p>{`© ${currentYear} Ukrainian Institute of Psychotherapy`}</p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-start gap-10">
          <div className="flex flex-col gap-1.5">
            ...
            <Link href="/accessibility" className={linkClass}>{"Доступність"}</Link>
          </div>
          <div className="flex flex-col gap-1.5">
        ...
        <p className="text-white/50 text-xs flex-shrink-0">{`© ${currentYear} Ukrainian Institute of Psychotherapy. `}{"Всі права захищені."}</p>
      </div>
    </footer>
  );
}