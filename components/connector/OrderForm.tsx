...
export default function OrderForm({ isOpen, onClose, labels }: OrderFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const amount = 1099;
      const selectedCountry = l.countries.find(c => c.code === formData.country);
      const orderRes = await fetch('/api/connector', {
        ...
        body: JSON.stringify({
          ...
          postOffice: formData.postOffice, amount, callMe: formData.callMe,
        }),
      });
      ...
      const paymentRes = await fetch('/api/wayforpay', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          orderReference, amount, productName: 'Гра Конектор',
          productPrice: amount, productCount: 1, clientEmail: formData.email,
        }),
      });
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {l.countryLabel} <span className="text-red-500">{"*"}</span>
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              {isUkraine ? (
                <UkraineDelivery
                  ...
                  showCities={showCities} showBranches={showBranches} labels={l}
                  onCityChange={(value) => { setFormData({ ...formData, city: value, postOffice: '' }); searchCities(value); }}
                  onCitySelect={(city) => { setFormData({ ...formData, city: city.Description, postOffice: '' }); setShowCities(false); setCities([]); }}
                  onPostOfficeChange={(value) => { setFormData({ ...formData, postOffice: value }); if (formData.city) searchBranches(formData.city, value); }}
                  onPostOfficeSelect={(branch) => { setFormData({ ...formData, postOffice: branch.Description }); setShowBranches(false); setBranches([]); }}
                  onPostOfficeFocus={() => { if (formData.city && formData.city.length >= 2) { searchBranches(formData.city, formData.postOffice); setShowBranches(true); } }}
                />
              ) : (
                <EuDelivery
                  city={formData.city} postOffice={formData.postOffice}
                  euCities={euCities} euDivisions={euDivisions}
                  loadingEu={loadingEu} showEuCities={showEuCities} showEuDivisions={showEuDivisions} labels={l}
                  onCityChange={(value) => { setFormData({ ...formData, city: value, postOffice: '' }); setEuDivisions([]); setShowEuDivisions(false); searchEuCities(value); }}
                  onCitySelect={(city) => { setFormData({ ...formData, city, postOffice: '' }); setShowEuCities(false); setEuCities([]); searchEuDivisions(city, ''); }}
                  onPostOfficeChange={(value) => { setFormData({ ...formData, postOffice: value }); searchEuDivisions(formData.city, value); }}
                  onPostOfficeFocus={() => { if (formData.city) searchEuDivisions(formData.city, formData.postOffice); }}
                  onDivisionSelect={(div) => { setFormData({ ...formData, postOffice: div.address || div.name }); setShowEuDivisions(false); setEuDivisions([]); }}
                />
              )}
