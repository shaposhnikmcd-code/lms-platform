...
Усі курси UIMP — **самостійні**, без стартової дати чи розкладу. Студент проходить у власному темпі після покупки. У будь-яких текстах (лендінги, листи, плейсхолдери, UI, описи) **не вживати** формулювання типу "курс скоро стартує", "запуск", "початок навчання", "розклад", "потік". Доступ відкривається одразу після оплати — **строк доступу в текстах не згадувати взагалі** (рішення Інституту 15.09.2026): ні "безстроковий", ні "довічний"/lifetime, ні конкретні терміни ("180 днів", "6 місяців", "пів року"). Просто не писати про строк доступу.
...
- **Плани**: `YEARLY` (15000 грн, доступ 365 днів) і `MONTHLY` (2200 грн, разова або автосписання). Місячний графік — **сітка модулів набору**: 9 модулів по календарному місяцю від `cohort.startDate` (для старту 01.09: вересень = модуль 1 … травень = модуль 9). Оплата всередині модуля покриває САМЕ цей модуль, незалежно від дня; наступні платежі/списання — у перший день наступних модулів. Сітка однакова для разових і автоплатежу; пізній покупець починає з пізнішого модуля і має менше слотів (`totalSlots = 9 − firstSlot`, купив у жовтні → 8 платежів). Правило краю: платіж менш ніж за 24 год до кінця модуля зараховується вже в наступний. Єдине джерело правди — `monthlySchedule` / `cohortSlotIndex` у [lib/yearlyProgramAccess.ts](lib/yearlyProgramAccess.ts); «9 − щось» ніде більше не рахувати, тести — [lib/yearlyProgramAccess.test.ts](lib/yearlyProgramAccess.test.ts) (`npm test`).
- **Статуси**: `PENDING` → `ACTIVE` → `GRACE` → `EXPIRED` / `CANCELLED`. Тривалість grace **конфігурується з адмінки** — ключ `yearlyGraceDays` в `AppSetting` (читається через `getYearlyGraceDays`, межі `YEARLY_GRACE_MIN_DAYS`…`YEARLY_GRACE_MAX_DAYS`); `YEARLY_PROGRAM_CONFIG.graceDays` — лише fallback, коли рядка в БД ще немає. Кількість днів НЕ хардкодити в текстах листів, FAQ і UI — брати з налаштування. Зміна впливає тільки на нові переходи ACTIVE→GRACE: у вже наявних записів межа зафіксована в `gracePeriodEndsAt`. Мінімум — **1 день** (`YEARLY_GRACE_MIN_DAYS`). При 1 дні розклад визначений явно: у день закінчення — лист «сьогодні останній день» і перехід у GRACE, наступного ранку — закриття доступу і лист про закриття; листа «пільговий період почався» при grace < 2 днів разова оплата НЕ отримує (`graceStartDecision` у [lib/yearlyProgramReminderSchedule.ts](lib/yearlyProgramReminderSchedule.ts)), автоплатіж — отримує свій «списання не пройшло».
- **Флоу перший платіж**: [CoursePurchaseModal](components/CoursePurchaseModal.tsx) → `/api/wayforpay` детектить префікс `yearly-program_` / `yearly-program-monthly_` через [yearlyProgramConfig.ts](lib/yearlyProgramConfig.ts) → створює/знаходить підписку (`PENDING`), для MONTHLY додає `regularOn=1, regularMode=monthly, dateNext, dateEnd` у payload (токенізація + автосписання на стороні WFP). `dateNext` = якір + 1 місяць, де **якір = початок модуля**, який покриває цей платіж (`cohortModuleStart(cohort, cohortSlotIndex(cohort, now))`, а якщо сплачено наперед — перший непокритий модуль). Тому дата списання у всіх однакова — день старту набору (набір 05.09 → списання 5-го числа), а не день покупки; `dateEnd` = початок останнього модуля набору + буфер. Поля `dateBegin` у Purchase WFP не існує — не використовувати.
...
- **Автоплатник зі зламаним списанням доплачує сам.** Rule 2 у `/api/wayforpay` («спочатку скасуйте автосписання») НЕ діє, коли підписка в `GRACE` або `failedChargeCount > 0` і оплата РАЗОВА — предикат `autopayAllowsManualTopUp` у [lib/yearlyProgramRenewState.ts](lib/yearlyProgramRenewState.ts), спільний для роуту, панелі поновлення і cron-листів. Відкриття такої оплати знімає WFP-правило і ставить `autoRenew=false` (гілка downgrade, подія `autorenew_downgraded` з `reason: failed_autopay_manual_topup`), щоб WFP не списав той самий модуль вдруге. Cyclical-листи (`cyclical-*`) несуть персональну кнопку `{payUrl}`, як і manual. Автоплатник у GRACE отримує cyclical-листи ЗАВЖДИ (не лише при відмові банку); перше речення `{reasonLine}` — правдива причина з `autopayGraceReason` ([lib/yearlyProgramReminderSchedule.ts](lib/yearlyProgramReminderSchedule.ts)): списання не пройшло / правила у WFP немає / графік зсунуто.
...
- **Адмінка**: `/dashboard/admin/yearly-program` — таблиця з KPI, фільтрами (план/статус/пошук), expandable row (клік по chevron) з трьома панелями: **Дії** (Продовжити/Скасувати автосписання/Закрити доступ/Відкрити знову/Видалити — POST на `/api/admin/yearly-program/[id]`; «🚫 Скасувати автосписання» (`cancel_autopay`) лише знімає WFP-правило і ставить `autoRenew=false` — статус ACTIVE/GRACE, Telegram і SendPulse без змін, далі йде ланцюг нагадувань разової оплати; повне припинення — «Закрити доступ» або «Деактивувати»), **Платежі** (список orderReference+сума+статус), **Події** (повний лог з `YearlyProgramSubscriptionEvent`). Endpoint `GET /api/admin/yearly-program/[id]/details` тягне все це разом.
