...
6. **Пакет FIXED_FREE з 1 безкоштовним курсом (1 або 2 платних)** — блок 690px, grid-cols-2: FreeMini `equalPair` + CTA-картка з 🎯 ГОТОВИЙ НАБІР badge, hero gradient ціна, кнопка "Купити пакет" `text-[18px] px-4 py-3 max-w-[90px]` в 2 рядки центровано. Стандартний нижній Price+CTA прихований. marginBottom: 0 на free-row.
...
9. **Пакет 🎲 Безкоштовний на Вибір з 1 платним + 3 безкоштовних на вибір** — гілка `isLargePaid` (ширина 58%/64%, h4 до 32px, опис 17px, прайс-смужка 9×16 padding + число 21px, hover free-карток з `translateY(-2px) !important` + light shadow). Всі три free-картки однакової висоти через flex-chain. Нижній Price+CTA блок `maxWidth: 480px`.
