...
import { FaTelegram, FaInstagram, FaYoutube, FaStar, FaCheck, FaWallet } from 'react-icons/fa';
            ...
            {[
              ...
              <div key={i} className="group relative bg-white rounded-2xl overflow-hidden shadow-2xl hover:shadow-3xl transition-all duration-500">
                <div className="relative h-[500px] w-full overflow-hidden">
                  <Image src={`/courses/psychology-basics/${teacher.image}.webp.webp`} alt={teacher.name} fill className="object-contain object-top group-hover:scale-110 transition-transform duration-700" sizes="(max-width: 768px) 100vw, 50vw" />
                ...
                <ul className="space-y-3">
                  {section.items.map((item, j) => (
                    <li key={j} className="flex items-start gap-3 text-gray-600">
                      <span className="w-6 h-6 bg-[#FDF2EB] rounded-full flex items-center justify-center text-[#D4A017] font-bold text-sm flex-shrink-0">{j + 1}</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="max-w-md mx-auto">
              <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-8 text-center border-2 border-[#D4A017]/30 hover:border-[#D4A017] transition-all">
                ...
                <div className="space-y-3 mb-8 text-white/80">
                  <p className="flex items-center justify-center gap-2"><FaCheck className="text-[#D4A017]" />Всі 15 лекцій</p>
                  <p className="flex items-center justify-center gap-2"><FaCheck className="text-[#D4A017]" />Доступ 6 місяців</p>
                  <p className="flex items-center justify-center gap-2"><FaCheck className="text-[#D4A017]" />Додаткові матеріали</p>
                </div>
                <WayForPayButton
      ...
      <footer className="bg-white border-t border-gray-100 py-16">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col items-center text-center">
            <p className="text-gray-600 mb-8 max-w-md">Залишились запитання? Ми завжди раді допомогти!</p>
            <a href="mailto:uimp.edu@gmail.com" className="text-xl text-[#D4A017] hover:text-[#b88913] transition-colors mb-8">
              uimp.edu@gmail.com
            </a>
            <div className="flex flex-wrap justify-center gap-4">
              {[
                ...
                <Link key={i} href={social.href} target="_blank"
                  ...
                  <span className="text-[#1C3A2E] group-hover:text-white transition-colors">{social.label}</span>
                </Link>
              ))}
            </div>
            <div className="mt-12 text-sm text-gray-400">© 2024 Ukrainian Institute of Mental Health. Всі права захищені.</div>
          </div>
        </div>
      </footer>
