...
const YEARLY_TG_GROUPS = new Set(['yearly-telegram']);
...
export async function GET(req: NextRequest) {
  ...
  const items = (Object.keys(PAYMENT_TEMPLATES) as PaymentTemplateKey[])
    .filter((key) => YEARLY_TG_GROUPS.has(PAYMENT_TEMPLATES[key].group))
    .map((key) => {
  ...
  const groups = PAYMENT_TEMPLATE_GROUPS.filter((g) => YEARLY_TG_GROUPS.has(g.id));
