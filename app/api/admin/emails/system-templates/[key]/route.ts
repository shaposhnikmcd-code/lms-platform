...
function isSystemKey(key: string): key is PaymentTemplateKey {
  return key in PAYMENT_TEMPLATES && PAYMENT_TEMPLATES[key as PaymentTemplateKey].group === 'system';
}
...
export async function GET(
  ...
  if (!isSystemKey(key)) {
    return NextResponse.json({ error: 'Unknown system template key' }, { status: 404 });
  }
