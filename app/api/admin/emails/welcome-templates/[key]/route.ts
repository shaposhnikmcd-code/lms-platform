...
function isWelcomeKey(key: string): key is PaymentTemplateKey {
  return key in PAYMENT_TEMPLATES && PAYMENT_TEMPLATES[key as PaymentTemplateKey].group === 'welcome';
}
...
export async function GET(
  ...
  if (!isWelcomeKey(key)) {
    return NextResponse.json({ error: 'Unknown welcome template key' }, { status: 404 });
  }
