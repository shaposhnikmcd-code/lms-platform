...
/// Bundle-templates CRUD. Структурно той самий, що /yearly-program/payment-templates/[key],
/// але звужений до групи `bundle` (поки лише ключ `bundle-purchase`). Зберігаємо у тій самій
/// таблиці `EmailTemplate` (single registry, спільний `templateKey` namespace).

function isBundleKey(key: string): key is PaymentTemplateKey {
  return key in PAYMENT_TEMPLATES && PAYMENT_TEMPLATES[key as PaymentTemplateKey].group === 'bundle';
}
...
export async function GET(
  ...
  if (!isBundleKey(key)) {
    return NextResponse.json({ error: 'Unknown bundle template key' }, { status: 404 });
  }
