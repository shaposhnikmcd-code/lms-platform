          ...
          {students.length === 0 ? (
            ...
            students.map((s) => (
              <button key={s.id} onClick={() => setSelectedStudent(s)}
                className={`w-full text-left p-3 flex items-center gap-3 hover:bg-gray-50 transition-colors border-b border-gray-50 ${selectedStudent?.id === s.id ? "bg-[#E8F5E0]" : ""}`}>
                ...
                <div className="overflow-hidden">
                  <p className="text-sm font-medium text-gray-800 truncate">{s.name || "Без імені"}</p>
          ...
          {!selectedStudent ? (
            ...
            <>
              <div className="p-4 border-b border-gray-100 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#1C3A2E]">{selectedStudent.name || "Без імені"}</p>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.length === 0 ? (
                  <p className="text-center text-gray-400 text-sm mt-8">{"Повідомлень ще немає"}</p>
                ) : (
                  messages.map((msg) => {
                    const isMine = msg.senderId === myId;
                    return (
                      <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${isMine ? "bg-[#1C3A2E] text-white rounded-br-none" : "bg-gray-100 text-gray-800 rounded-bl-none"}`}>
                          <p>{msg.text}</p>
