    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="text-2xl font-bold text-[#1C3A2E] mb-1">{courseTeachers.length}</div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        {courseTeachers.length === 0 ? (
          <p className="text-gray-400 text-sm">{"Вам ще не призначено жодного курсу. Зверніться до адміністратора."}</p>
        ) : (
          <div className="space-y-4">
            {courseTeachers.map((ct) => (
              <div key={ct.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                <div>
                  <p className="font-medium text-[#1C3A2E]">{ct.course.title}</p>
                  <p className="text-sm text-gray-500">{ct.course._count.enrollments} {"студентів"}</p>
                </div>
