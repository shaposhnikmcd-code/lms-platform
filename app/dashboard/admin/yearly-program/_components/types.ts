...
export interface Row {
  ...
  note: string | null;
  /// Стан сітки модулів набору для MONTHLY-підписки. null — річна, або набору немає
  /// (legacy-рядок), і питання «скільки модулів сплачено» не має відповіді.
  ///
  /// ЄДИНЕ джерело для «сплачено повністю» у таблиці. Лічильник `paymentsCount >= 9`
  /// тут брехав: пізній покупець стартує з пізнішого модуля і має менше слотів
  /// (купив у жовтні → 8 модулів), тож 8/8 — це повна оплата, а не борг.
  schedule: RowSchedule | null;
}

export interface RowSchedule {
  /// Модуль першої оплати (0-based) — 0 для того, хто зайшов з першого модуля набору.
  firstSlot: number;
  /// Скільки модулів має сплатити САМЕ ця підписка.
  totalSlots: number;
  paidCount: number;
  isFullyPaid: boolean;
  /// Скільки модулів у наборі взагалі (знаменник у «3 з 9»).
  moduleCount: number;
  /// 1-based номер наступного неоплаченого модуля; null — усе сплачено.
  nextModuleNumber: number | null;
  /// «листопад 2026» для наступного неоплаченого модуля; null — усе сплачено.
