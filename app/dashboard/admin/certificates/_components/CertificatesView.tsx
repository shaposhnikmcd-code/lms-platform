...
interface CourseCandidate {
  ...
  courseTitle: string;
  enrolledAt: string;
  certificate: { certNumber: string; emailStatus: EmailStatus } | null;
}
...
export default function CertificatesView() {
  ...
  const [activeTab, setActiveTabState] = useState<TabKey>(() => {
    ...
    if (t === 'courses' || t === 'yearly' || t === 'history' || t === 'analytics') return t;
    return 'courses';
  });
...
function CoursesTab({
  ...
  const fetchList = useCallback(async () => {
    setLoading(true);
    const qs = new URLSearchParams({ type: 'COURSE' });
  ...
  async function handleResend(id: string) {
    setBusyId(id);
    try {
      const res = await fetch(`/api/admin/certificates/${id}/resend`, { method: 'POST' });
      const data = await res.json();
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap mb-5">
        <div className="flex items-center gap-2 flex-wrap">
          <input
            ...
            placeholder="Пошук по імені, email або номеру…"
          ...
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as 'all' | EmailStatus)}
...
function YearlyTab({
  ...
  const dark = theme === 'dark';
  const [candidates, setCandidates] = useState<YearlyCandidate[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
          ...
          <tbody className={dark ? 'divide-y divide-white/[0.04]' : 'divide-y divide-stone-200/60'}>
            ...
            {filtered.map((c) => (
              <tr key={c.subscriptionId} className={dark ? 'hover:bg-white/[0.02]' : 'hover:bg-stone-50/70'}>
                ...
                <td className="py-3 pr-3">{c.expiresAt ? formatDate(c.expiresAt) : '—'}</td>
                <td className="py-3 pr-3">
                  {c.certificate ? (
                    <div>
  ...
  return (
    <div className="overflow-x-auto rounded-xl">
      <table className="w-full text-[13px]">
        ...
        <tbody className={dark ? 'divide-y divide-white/[0.04]' : 'divide-y divide-stone-200/60'}>
          {loading && (
            <tr><td colSpan={showCourse ? 7 : 6} className="py-8 text-center text-stone-500">Завантаження…</td></tr>
          )}
          {!loading && items.length === 0 && (
            <tr><td colSpan={showCourse ? 7 : 6} className="py-8 text-center text-stone-500">Сертифікатів немає</td></tr>
          )}
          {items.map((c) => (
            <tr key={c.id} className={dark ? 'hover:bg-white/[0.02]' : 'hover:bg-stone-50/70'}>
              <td className="py-3 pr-3 font-mono text-[11px]">{c.certNumber}</td>
              <td className="py-3 pr-3">
              ...
              {showCourse && <td className="py-3 pr-3">{c.courseName ?? '—'}</td>}
              <td className="py-3 pr-3 whitespace-nowrap">{formatDate(c.issuedAt)}</td>
              <td className="py-3 pr-3">
                {c.issuedManually ? (
                  <div>
                    <div className="text-[12px]">{c.issuedByName ?? '—'}</div>
                    <div className={`text-[10px] ${dark ? 'text-slate-400' : 'text-stone-500'}`}>{c.issuedByEmail ?? ''}</div>
                  </div>
                ) : (
                  <span className={`text-[11px] italic ${dark ? 'text-slate-500' : 'text-stone-400'}`}>Авто (cron)</span>
                )}
              </td>
              <td className="py-3 pr-3">
                <StatusBadge theme={theme} status={c.emailStatus} revoked={c.revoked} />
                {c.emailError && (
                  <div className={`text-[10px] mt-0.5 ${dark ? 'text-red-300' : 'text-red-700'}`} title={c.emailError}>
                    {c.emailError.length > 40 ? c.emailError.slice(0, 40) + '…' : c.emailError}
                  </div>
                )}
              </td>
              <td className="py-3">
                <div className="inline-flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    disabled={busyId === c.id || c.revoked}
                    onClick={() => onResend(c.id)}
                    title={c.revoked ? 'Не можна перевідправити — відкликано' : 'Перевідправити'}
