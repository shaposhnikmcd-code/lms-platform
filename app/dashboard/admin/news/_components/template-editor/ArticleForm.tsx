...
// Гнучкість:
//  - Toggle show/hide для кожної секції (`data.hidden[region]`).
//  - Reorder для рухомих секцій (`ARTICLE_MOVABLE_REGIONS` = sections, pullquote,
//    conclusion, author). Cover і header закріплені зверху.
// Підтримка через RegionGroup wrap-и: focus у формі → highlight у preview.
...
export default function ArticleForm({ data, onChange, onFocusRegion }: Props) {
  ...
  // Show/hide toggle
  const isHidden = (region: ArticleRegionKey) => data.hidden?.[region] === true;
  const toggleHidden = (region: ArticleRegionKey) => {
  ...
  // Reorder movable regions. Returns mover-функції для конкретного region.
  const movableOrder: ArticleRegionKey[] = (resolveArticleOrder(data.order).filter(k =>
    ARTICLE_MOVABLE_REGIONS.includes(k)
  ) as ArticleRegionKey[]);

  const moveMovableRegion = (region: ArticleRegionKey, dir: -1 | 1) => {
    const idx = movableOrder.indexOf(region);
    const target = idx + dir;
    if (idx < 0 || target < 0 || target >= movableOrder.length) return;
    const next = [...movableOrder];
    [next[idx], next[target]] = [next[target], next[idx]];
    onChange({ ...data, order: next });
  };

  const movableProps = (region: ArticleRegionKey) => {
    const idx = movableOrder.indexOf(region);
    return {
      onMoveUp: () => moveMovableRegion(region, -1),
      onMoveDown: () => moveMovableRegion(region, 1),
      canMoveUp: idx > 0,
      canMoveDown: idx >= 0 && idx < movableOrder.length - 1,
    };
  };
  ...
  // Render renderers — рендеримо секції в effective order для menager UX.
  // Внутрішньо preview render використовує цей же order (через resolveArticleOrder
  // у ArticleTemplate). Менеджер бачить форму в тому ж порядку, що й картку.
  const orderForUI = resolveArticleOrder(data.order);

  const renderRegion = (region: ArticleRegionKey): React.ReactNode => {
    switch (region) {
      case "cover":
        return (
          <RegionGroup key="cover" region="cover" onFocusRegion={onFocusRegion}>
            <SectionHeader
              icon="🖼"
              title="Cover"
              whereOnCard="hero, 16:9"
              hidden={isHidden("cover")}
              onToggleHidden={() => toggleHidden("cover")}
            />
            {!isHidden("cover") && (
              <ImageInput
                label="Головне фото"
      ...
      case "sections":
        return (
          <RegionGroup key="sections" region="sections" onFocusRegion={onFocusRegion}>
            <SectionHeader
              icon="📑"
              title={`Розділи (${data.sections.length})`}
              whereOnCard="body статті"
              hidden={isHidden("sections")}
              onToggleHidden={() => toggleHidden("sections")}
              {...movableProps("sections")}
            />
            {!isHidden("sections") && (
      ...
      case "pullquote":
        return (
          <RegionGroup key="pullquote" region="pullquote" onFocusRegion={onFocusRegion}>
            <SectionHeader
              icon="💬"
              title="Pull-quote"
              whereOnCard="між розділами"
              hidden={isHidden("pullquote")}
              onToggleHidden={() => toggleHidden("pullquote")}
      ...
      case "conclusion":
        return (
          <RegionGroup key="conclusion" region="conclusion" onFocusRegion={onFocusRegion}>
            <SectionHeader
              icon="🎯"
              title="Висновки"
              whereOnCard="низ статті"
              hidden={isHidden("conclusion")}
              onToggleHidden={() => toggleHidden("conclusion")}
      ...
      case "author":
        return (
          <RegionGroup key="author" region="author" onFocusRegion={onFocusRegion}>
            <SectionHeader
              icon="✒️"
              title="Footer"
              whereOnCard="підпис унизу"
              hidden={isHidden("author")}
              onToggleHidden={() => toggleHidden("author")}
              {...movableProps("author")}
            />
            {!isHidden("author") && (
              <TextInput
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {orderForUI.map(region => renderRegion(region))}
    </div>
  );
}
