"use client";

import { useEffect, useState } from "react";
import dynamic from "next/dynamic";
import { useRouter, useParams } from "next/navigation";
import { NewsMeta } from "../../_components/editor/types";
import { PREVIEW_CARD_WIDTH } from "@/lib/news/render";

// Білдер «Превʼю картки новини».
//
// Канвас має фіксовану ширину PREVIEW_CARD_WIDTH (360px) — стандартний
// розмір картки на /news. Менеджер бачить WYSIWYG: те що верстає тут =
// те що рендериться у newsCard-блоках публічного /news 1-в-1
// (через AbsoluteBlockRender у lib/news/render.tsx).
//
// Зберігається у `News.previewContent`. Meta (title/slug/excerpt/category/
// imageUrl/pageBgColor) редагується через MetaSidebar справа.
// `News.content` (повна стаття) НЕ торкається — редагується у `/[id]/edit`.

const NewsEditor = dynamic(() => import("../../_components/editor/NewsEditor"), {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center h-64">
      <div className="w-8 h-8 border-2 border-[#D4A843] border-t-transparent rounded-full animate-spin" />
    </div>
  ),
});

export default function EditNewsPreviewPage() {
  const router = useRouter();
  const params = useParams();
  const id = Array.isArray(params.id) ? params.id[0] : (params.id as string);

  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [initialMeta, setInitialMeta] = useState<Partial<NewsMeta>>({});
  const [initialPreviewContent, setInitialPreviewContent] = useState("");

  useEffect(() => {
    if (!id) return;
    fetch("/api/admin/news/" + id)
      .then(r => { if (!r.ok) throw new Error("HTTP " + r.status); return r.json(); })
      .then(d => {
        setInitialMeta({
          title: d.title || "",
          slug: d.slug || "",
          excerpt: d.excerpt || "",
          category: d.category || "NEWS",
          imageUrl: d.imageUrl || "",
          pageBgColor: d.pageBgColor || "",
          published: d.published || false,
        });
        setInitialPreviewContent(d.previewContent || "");
        setLoading(false);
      })
      .catch(e => { setError("Помилка завантаження: " + e.message); setLoading(false); });
  }, [id]);

  // ВАЖЛИВО: параметр `content` тут — це JSON `previewContent` (бо mode="preview"
  // означає, що канвас редагує превʼю-картку, а не основний контент статті).
  // PATCH відправляє його у поле `previewContent` БД — `content` НЕ перетираємо.
  const handleSave = async (meta: NewsMeta, content: string, imageUrl: string) => {
    setSaving(true);
    try {
      const res = await fetch("/api/admin/news/" + id, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: meta.title,
          slug: meta.slug,
          excerpt: meta.excerpt,
          category: meta.category,
          imageUrl,
          pageBgColor: meta.pageBgColor || null,
          previewContent: content,
        }),
      });
      if (res.ok) {
        router.push("/dashboard/admin/news");
        return;
      }
      const body = await res.json().catch(() => ({}));
      throw new Error(body?.error || `Помилка збереження (HTTP ${res.status})`);
    } finally {
      setSaving(false);
    }
  };

  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "256px" }}>
      <div style={{ width: "32px", height: "32px", borderWidth: "4px", borderStyle: "solid", borderColor: "#1C3A2E", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
    </div>
  );

  if (error) return (
    <div style={{ padding: "24px" }}>
      <div style={{ background: "#FEF2F2", borderWidth: "1px", borderStyle: "solid", borderColor: "#FECACA", borderRadius: "12px", padding: "24px", color: "#DC2626" }}>{error}</div>
    </div>
  );

  return (
    <NewsEditor
      pageTitle="Превʼю картки новини"
      initialMeta={initialMeta}
      initialContent={initialPreviewContent}
      newsId={id}
      mode="preview"
      canvasWidth={PREVIEW_CARD_WIDTH}
      minCanvasHeight={400}
      fixedHeight
      canvasLabel={{
        left: "🃏 Превʼю-картка",
        right: `${PREVIEW_CARD_WIDTH} × 400 px — фіксовано`,
      }}
      onSave={handleSave}
      onBack={() => router.push("/dashboard/admin/news")}
      saving={saving}
    />
  );
}
